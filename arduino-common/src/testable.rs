...
impl Testable{
    pub fn new(error_rate: f64, omission_rate: f64)->(Self, Self){
        ...
        let master = Self{tx: master_tx, rx: master_rx, error_rate, omission_rate, rng: thread_rng()};
        let slave = Self{tx: slave_tx, rx: slave_rx, error_rate, omission_rate, rng: thread_rng()};
        (master, slave)
    }
}
